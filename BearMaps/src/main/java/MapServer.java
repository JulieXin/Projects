...
public class MapServer {
    ...
    private static LinkedList<Node> nodesLinkedList;
    ...
    public static void main(String[] args) {
        ...
        /* Define the API endpoint for findList */
        get("/findList", (req, res) -> {
    ...
    public static Map<String, Object> getMapRaster(Map<String, Double> params, OutputStream os) {
        ...
        /* finds depth of wanted nodes based off of queryDDP and tileDDP */
        double queryDDP = (params.get("lrlon") - params.get("ullon")) / params.get("w");
        int level = 0;
        double tileDDP = (ROOT_LRLON - ROOT_ULLON) / (Math.pow(2, level) * TILE_SIZE);
        while (tileDDP > queryDDP) {
            level += 1;
            tileDDP = (ROOT_LRLON - ROOT_ULLON) / (Math.pow(2, level) * TILE_SIZE);
        }

        Rectangle2D.Double queryWindow = new Rectangle2D.Double((params.get("ullon")),
                (params.get("ullat")), (params.get("lrlon") - params.get("ullon")),
                (params.get("ullat") - params.get("lrlat")));

        ArrayList<QuadTree.QuadTreeNode> nodeList =  quadTree.traverse(queryWindow,
                quadTree.getRoot(), level);

        int width = 1;
        for (int i = 0; i < nodeList.size() - 1; i++) {
            if (nodeList.get(i).getUllon() < nodeList.get(i + 1).getUllon()) {
                width += 1;
            } else {
                break;
            }
        ...
        if (!nodeList.isEmpty()) {
            int height = nodeList.size() / width;
            int x = 0, y = 0;
            int rasterWidth = width * TILE_SIZE;
            int rasterHeight = height * TILE_SIZE;
            /* setting up graphics/imaging for final image and routing line */
            BufferedImage im = new BufferedImage(rasterWidth, rasterHeight,
                    BufferedImage.TYPE_INT_RGB);
            Graphics graphics = im.getGraphics();
            graphics.setColor(ROUTE_STROKE_COLOR);
            ((Graphics2D) graphics).setStroke(new BasicStroke(1.5f));
            for (QuadTree.QuadTreeNode node : nodeList) {
                try {
                    int id = node.getImageId();
                    String fileName = String.valueOf(id);
                    if (fileName.equals("0")) {
                        fileName = "root";
                    }
                    fileName = IMG_ROOT + fileName + ".png";
            ...
            ((Graphics2D) graphics).setStroke(new BasicStroke(ROUTE_STROKE_WIDTH_PX,
                    BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
            QuadTree.QuadTreeNode starterNode = nodeList.get(0);
            drawConnection(graphics, starterNode);
            try {
                int lastNodeNum = nodeList.size() - 1;
                ImageIO.write(im, "png", os);
                rasteredImageParams.put("raster_ul_lon", starterNode.getUllon());
                rasteredImageParams.put("raster_ul_lat", starterNode.getUllat());
                rasteredImageParams.put("raster_lr_lon", nodeList.get(lastNodeNum).getLrlon());
                rasteredImageParams.put("raster_lr_lat", nodeList.get(lastNodeNum).getLrlat());
                rasteredImageParams.put("raster_width", rasterWidth);
                rasteredImageParams.put("raster_height", rasterHeight);
                rasteredImageParams.put("depth", starterNode.getDepth());
    ...
    private static void drawConnection(Graphics graphics, QuadTree.QuadTreeNode starterNode) {
        if (nodesLinkedList != null) {
            double starterUllon = starterNode.getUllon();
            double starterUllat = starterNode.getUllat();
            double starterLrlon = starterNode.getLrlon();
            double starterLrlat = starterNode.getLrlat();

            double xDPP = (starterUllon - starterLrlon) / TILE_SIZE;
            double yDPP = (starterLrlat - starterUllat) / TILE_SIZE;

            for (int i = 0; i < nodesLinkedList.size() - 1; i++) {
                Node node = nodesLinkedList.get(i);
                Node nextNode = nodesLinkedList.get(i + 1);

                /* x & y based on current node */
                double nodeX = (starterUllon - node.getLon()) / xDPP;
                double nodeY = (node.getLat() - starterUllat) / yDPP;

                /* x & y based on next node */
                double nextNodeX = (starterUllon - nextNode.getLon()) / xDPP;
                double nextNodeY = (nextNode.getLat() - starterUllat) / yDPP;
                graphics.drawLine((int) nodeX, (int) nodeY, (int) nextNodeX, (int) nextNodeY);
            }
        }
    }
    ...
    public static LinkedList<Long> findAndSetRoute(Map<String, Double> params) {
        nodesLinkedList = g.findRoute(params);

        LinkedList<Long> routes = new LinkedList<>();
        for (Node node : nodesLinkedList) {
            routes.add(node.getId());
        }
        return routes;
    }
